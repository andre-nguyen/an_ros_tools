...
int main(int argc, char** argv) {
  ...
  typedef sync_policies::ApproximateTime <geometry_msgs::PoseStamped, geometry_msgs::PoseStamped> PoseStampedSyncPolicy;
  typedef sync_policies::ApproximateTime <nav_msgs::Odometry, nav_msgs::Odometry> OdomSyncPolicy;

  Subscriber<geometry_msgs::PoseStamped> pose_sub1(nh, "pose1", 1);
  Subscriber<geometry_msgs::PoseStamped> pose_sub2(nh, "pose2", 1);
  Synchronizer<PoseStampedSyncPolicy> sync(PoseStampedSyncPolicy(10), pose_sub1, pose_sub2);
  sync.registerCallback(boost::bind(&callback, _1, _2));

  Subscriber<nav_msgs::Odometry> odom_sub1(nh, "odom1", 1);
  Subscriber<nav_msgs::Odometry> odom_sub2(nh, "odom2", 1);
  Synchronizer<OdomSyncPolicy> sync2(OdomSyncPolicy(10), odom_sub1, odom_sub2);
  ...
  odom_pub = nh.advertise<nav_msgs::Odometry> ("odom_diff", 10);
